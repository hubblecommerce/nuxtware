...
import type { CmsElementSidebarFilter, UseListingReturn } from '@shopware/composables'
...
// Safely inject category listing context (undefined when not inside a category listing parent)
const categoryListing = inject<UseListingReturn | undefined>('categoryListing', undefined)

// Always-available singleton — does not require any parent context
const searchListing = useProductSearchListing()

const isSearchListing = computed(() => props.listingType === 'search')

const getInitialFilters = computed(() =>
    isSearchListing.value
        ? searchListing.getInitialFilters.value
        : (categoryListing?.getInitialFilters.value ?? [])
)

const getCurrentSortingOrder = computed(() =>
    isSearchListing.value
        ? searchListing.getCurrentSortingOrder.value
        : categoryListing?.getCurrentSortingOrder.value
)

const filtersToQuery = (criteria: RequestParameters<'searchPage'>) =>
    isSearchListing.value
        ? searchListing.filtersToQuery(criteria)
        : (categoryListing?.filtersToQuery(criteria) ?? {})

const doSearch = (criteria: RequestParameters<'searchPage'>) =>
    isSearchListing.value
        ? searchListing.search(criteria)
        : (categoryListing?.search(criteria) ?? Promise.resolve())
...
const searchCriteriaForRequest: ComputedRef<RequestParameters<'searchPage'>> =
    computed(() => ({
        ...
        search: isSearchListing.value ? (route.query.search as string || '') : '',
...
const buildRouteQuery = () => ({
    ...
    ...(isSearchListing.value ? { search: route.query.search } : {}),
})
...
const onOptionSelectToggle = async ({
    ...
    await router.push({ query: buildRouteQuery() } as RouteLocationRaw)
    await doSearch({
